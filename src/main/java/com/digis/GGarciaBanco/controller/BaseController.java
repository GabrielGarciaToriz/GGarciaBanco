...
public abstract class BaseController {
    ...
    private HttpStatus resolverStatus(Result<?> result) {
        ...
        return switch (result.errorCode) {
            case ErrorCode.NOT_FOUND             -> HttpStatus.NOT_FOUND;             // 404
            case ErrorCode.DUPLICATE             -> HttpStatus.CONFLICT;              // 409
            case ErrorCode.INVALID_INPUT         -> HttpStatus.UNPROCESSABLE_ENTITY;  // 422
            case ErrorCode.INTERNAL_ERROR        -> HttpStatus.INTERNAL_SERVER_ERROR; // 500
            case ErrorCode.ACCOUNT_NOT_ACTIVATED -> HttpStatus.FORBIDDEN;             // 403
